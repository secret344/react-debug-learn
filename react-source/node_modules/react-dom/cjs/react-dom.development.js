...
// 用于修补 console.logs 的帮助程序，以避免在无副作用的
// 在渲染函数上进行重放。目前，这只是懒惰地修补对象，
// 如果急切地提取日志函数，则无法覆盖。我们也可以急切地修补该方法。
var disabledDepth = 0;
var prevLog;
var prevInfo;
var prevWarn;
...

{
  ...
  if (typeof window !== 'undefined' && typeof window.dispatchEvent === 'function' && typeof document !== 'undefined' && typeof document.createEvent === 'function') {
    ...
    invokeGuardedCallbackImpl = function invokeGuardedCallbackDev(name, func, context, a, b, c, d, e, f) {
      ...
      // 同步地派发我们的假事件。如果用户提供的函数出错，它将触发我们的全局错误处理程序。
...
function getNextLanes(root, wipLanes) {
  ...
  // 在所有非空闲的工作完成之前，不要从事任何空闲的工作，即使工作暂停
  ...
  if (nonIdlePendingLanes !== NoLanes) {
    ...
    // The only remaining work is Idle .唯一剩下的工作就是“空闲”。
  ...
  // 如果我们已经暂停了一段时间，那么打断也没关系。不要等待根完成。
...
function markRootUpdated(root, updateLane, eventTime) {
  ...
  // them again.
  // 如果有任何暂停的过渡，有可能这个新的更新可以解除它们。
  // 清除暂停的通道，这样我们就可以再次尝试渲染它们。
  // TODO: We really only need to unsuspend only lanes that are in the
  // `subtreeLanes` of the updated fiber, or the update lanes of the return
  // path. This would exclude suspended updates in an unrelated sibling tree,
  // since there's no way for this update to unblock it.
  // 我们实际上只需要取消暂停在更新光纤的`子树通道`中的通道，
  // 或者返回路径的更新通道。这将排除不相关的子树中被暂停的更新，因为这个更新没有办法解封它。
  // We don't do this if the incoming update is idle, because we never process
  // idle updates until after all the regular updates have finished; there's no
  // way it could unblock a transition.
  //  如果传入的更新是空闲的，我们不会这样做，
  // 因为我们从不处理空闲的更新，直到所有的常规更新完成之后；它不可能解开一个过渡。
  ...
  // 我们总是可以覆盖现有的时间戳，因为我们更喜欢最近的事件，并且我们假设时间是单调递增的。
  eventTimes[index] = eventTime;
}
function markRootSuspended(root, suspendedLanes) {
  root.suspendedLanes |= suspendedLanes;
  root.pingedLanes &= ~suspendedLanes; // The suspended lanes are no longer CPU-bound. Clear their expiration times.
  //  暂停的车道不再是CPU绑定的。清除它们的过期时间。
...
function dispatchEvent(domEventName, eventSystemFlags, targetContainer, nativeEvent) {
  ...
  if (allowReplay && hasQueuedDiscreteEvents() && isReplayableDiscreteEvent(domEventName)) {
    ...
    // 如果我们已经有一个离散事件队列，而这是另一个离散事件，那么我们不能不管它的目标是什么来分派它，因为它们需要按顺序分派。
...
function listenToAllSupportedEvents(rootContainerElement) {
  if (!rootContainerElement[listeningMarker]) {
    ...
    if (ownerDocument !== null) {
      ...
      // selectionchange事件也需要重复数据删除，但它附加到文档。
...
function setInitialProperties(domElement, tag, rawProps, rootContainerElement) {
  ...
  switch (tag) {
    ...
    case 'link':
      // We listen to these events in case to ensure emulated bubble
      // listeners still fire for error and load events. 我们监听这些事件，以确保模拟的泡沫监听器仍然对错误和加载事件起作用。
      listenToNonDelegatedEvent('error', domElement);
      listenToNonDelegatedEvent('load', domElement);
      props = rawProps;
      break;
  ...
  switch (tag) {
    ...
    default:
      if (typeof props.onClick === 'function') {
        ...
        //  对于SVG、MathML或自定义元素来说，这种cast可能不健全。
...
function getRootHostContext(rootContainerInstance) {
  var type;
  var namespace;
  var nodeType = rootContainerInstance.nodeType; //根节点实例 元素类型
...
function push(cursor, value, fiber) {
  ...
  // 将上一个value 推入valueStack ，当前的fiber推入fiberStack 一一对应
...
/** 是否执行工作堆栈游标 指向布尔值的游标，指示上下文是否已更改。 */ 
...
function pushTopLevelContextObject(fiber, context, didChange) {
  {
    ...
    // 添加上下文 cursor.current = value 
    // valueStack添加当前current fiberStack添加当前fiber
...
function scheduleSyncCallback(callback) {
  ...
  // 把这个回调推到一个内部队列中。我们会在下一个tick中刷新这些回调，
  // 如果有东西调用`flushSyncCallbackQueue'，则会提前刷新。
  if (syncQueue === null) {
    syncQueue = [callback];
  } else {
    // Push onto existing queue. Don't need to schedule a callback because
    // we already scheduled one when we created the queue.
    // 推送到现有的队列中。不需要安排回调，因为我们在创建队列时已经安排了一个回调。
...
function flushSyncCallbacks() {
  if (!isFlushingSyncQueue && syncQueue !== null) {
    ...
    try {
      ...
      // 这还有必要吗？在这个队列中运行的唯一用户代码是在渲染或提交阶段。
      ...
      // 如果有东西抛出，就把剩余的回调留在队列中。
      if (syncQueue !== null) {
        syncQueue = syncQueue.slice(i + 1);
      } // Resume flushing in the next tick
      //  在 next tick时恢复 flushing
...
// of the queue.
//  在当前渲染过程中收到更新的所有更新队列的一个数组。
// 当这个渲染退出时，无论是因为它完成了还是因为它被打断了，
// 交错的更新将被转移到队列的主要部分。
...
function enqueueInterleavedUpdates() {
  ...
  // 将交错更新传输到主队列上。每个队列都有一个“挂起”字段和一个“交错”字段。当它们不为空时，
  // 它们指向循环链表中的最后一个节点。我们需要将交叉列表添加到待处理列表的末尾，方法是将它们连接到一个单一的循环列表中。
  if (interleavedQueues !== null) {
...
// 全局状态，在调用' processUpdateQueue '开始时被重置。
// 它应该只在通过' checkHasForceUpdateAfterProcessing '调用' processUpdateQueue '之后读取。
...
function processUpdateQueue(workInProgress, props, instance, renderLanes) {
  ...
  // 检查是否有待定的更新。如果是，将它们转移到基队列。
  var pendingQueue = queue.shared.pending;

  if (pendingQueue !== null) {
    queue.shared.pending = null; // The pending queue is circular. Disconnect the pointer between first
    // and last so that it's non-circular. 
    // 挂起队列是循环的。断开第一个和最后一个之间的指针，使其是非循环的。
    ...
    // 如果存在一个当前队列，并且它不同于基本队列，那么我们也需要将更新传输到该队列。
    // 因为基本队列是一个没有循环的单链接列表，所以我们可以同时添加到这两个列表并利用结构共享。
    // TODO:传递“current”作为参数
    var current = workInProgress.alternate; //current fiber共享updataqueue
  ...
  if (firstBaseUpdate !== null) {
    ...
    // 不需要积累这个。相反，我们可以从原始的车道中移除renderLanes。
    ...
    do {
      ...
      if (!isSubsetOfLanes(renderLanes, updateLane)) {
        ...
        // update/state. 首要任务是不够的。跳过这个更新。如果这是第一个跳过的更新，那么前面的更新/状态就是新的基本更新/状态。
        var clone = {
    ...
    // are remaining.
    // 交错更新存储在一个单独的队列中。我们不打算在渲染期间处理它们，但我们确实需要跟踪哪些车道还在。
...
function ChildReconciler(shouldTrackSideEffects) {
  // shouldTrackSideEffects首次挂载为false
  ...
  function deleteRemainingChildren(returnFiber, currentFirstChild) {
    ...
    // 对于shouldClone的情况，这可以通过假设在第一个孩子之后我们已经添加了所有的东西来进行微观优化。
  ...
  function placeSingleChild(newFiber) {
    ...
    // 对于单个子节点的情况，这更简单。我们只需要为插入新的子元素做一个位置。
  ...
  function reconcileChildrenArray(returnFiber, currentFirstChild, newChildren, lanes) {
    ...
    if (oldFiber === null) {
      ...
      // 如果我们没有更多现有的孩子，我们可以选择一个快速路径，因为其余的都是插入。 
      for (; newIdx < newChildren.length; newIdx++) { // 创建fiber
        ...
        if (previousNewFiber === null) {
          // TODO: Move out of the loop. This only happens for the first run.  
          // 从循环中移出。这只发生在第一次运行时。
          resultingFirstChild = _newFiber; //首个child fiber
        } else {
          previousNewFiber.sibling = _newFiber; // 链接兄弟fiber
        }
    ...
    if (shouldTrackSideEffects) {
      ...
      // 任何现有的没有被上述消耗的孩子都被删除。我们需要将它们添加到删除列表中。
  ...
  function reconcileSingleTextNode(returnFiber, currentFirstChild, textContent, lanes) {
    ...
    if (currentFirstChild !== null && currentFirstChild.tag === HostText) {
      ...
      // the rest.
      // 我们已经有了一个现有的节点，所以我们只需更新它，并删除其他部分。
  ...
  function reconcileSingleElement(returnFiber, currentFirstChild, element, lanes) {
    var key = element.key;
    // 当前首个子节点 root节点不存在child
    var child = currentFirstChild;
    // 遍历判断之前子节点是否存在当前key 存在复用 不存在标记删除
    while (child !== null) {
    ...
    if (element.type === REACT_FRAGMENT_TYPE) {
      ...
      // 创建fiber root节点创建的是render时传入的节点fiber
  ...
  function reconcileChildFibers(returnFiber, currentFirstChild, newChild, lanes) {
    ...
    // 如果是react的文档片段节点（Frament）react.fragment
    var isUnkeyedTopLevelFragment = typeof newChild === 'object' && newChild !== null && newChild.type === REACT_FRAGMENT_TYPE && newChild.key === null;
    // 取他的子节点作为新的节点，react.fragment不渲染
...
/**
 * 上下文堆栈指针
 * 首次为  ｛current: {namespace,ancestorInfo}｝根节点信息 
 */
 ...
 * @param {*} nextRootInstance 首次为root节点示例
 */
function pushHostContainer(fiber, nextRootInstance) {
  // Push current root instance onto the stack;
  // This allows us to reset root when portals are popped.
  // 将当前根实例推入堆栈;这允许我们在门户弹出时重置root。
  push(rootInstanceStackCursor, nextRootInstance, fiber); // Track the context and the Fiber that provided it.
  // This enables us to pop only Fibers that provide unique contexts.
  // 跟踪上下文和提供它的Fiber。这使我们能够只弹出提供惟一上下文的光纤。
  push(contextFiberStackCursor, fiber, fiber); // Finally, we need to push the host context to the stack.
  // However, we can't just call getRootHostContext() and push it because
  // we'd have a different number of entries on the stack depending on
  // whether getRootHostContext() throws somewhere in renderer code or not.
  // So we push an empty value first. This lets us safely unwind on errors.
  // 最后，我们需要将宿主上下文推入堆栈。然而，我们不能只是调用getRootHostContext()并推入它，
  // 因为我们会根据getRootHostContext()是否在渲染器代码中抛出某个地方而在堆栈上有不同数量的条目。
  // 所以我们首先输入一个空值。这让我们可以安全地在错误上展开。
  push(contextStackCursor$1, NO_CONTEXT, fiber);
  var nextRootContext = getRootHostContext(nextRootInstance); // Now that we know this function doesn't throw, replace it.
  // 既然我们知道这个函数不会抛出，那就替换它。
...
//  除非是独特的，否则不要push这个Fiber的上下文 。
  if (context === nextContext) {
    return;
  } // Track the context and the Fiber that provided it.
  // This enables us to pop only Fibers that provide unique contexts.
  //  追踪上下文和提供它的Fiber。这使我们能够只弹出提供独特上下文的Fiber。
...
// 新挂载的缓存边界应该使用的缓存。它要么从缓存池中检索，要么从刷新的结果中检索。
...
function renderWithHooks(current, workInProgress, Component, props, secondArg, nextRenderLanes) {
  ...
  workInProgress.lanes = NoLanes; // The following should have already been reset 以下内容应该已经被重置
  // currentHook = null;
  ...
  // 如果挂载期间根本没有使用钩子，则发出警告，然后在更新期间使用一些钩子。
  // 目前我们将把更新渲染标识为挂载，因为memoizedState === null。
  // 使用memoizedState来区分挂载/更新只有在至少一个有状态钩子被使用的情况下才有效。
  // 非状态钩子(例如context)不会被添加到memoizedState中，所以在更新和挂载期间，memoizedState将为空。
  {
    if (current !== null && current.memoizedState !== null) {
      ...
      // 这个调度器处理一个边缘情况，即一个组件正在更新 但没有使用有状态钩子 。
      // 我们希望与生产代码的行为相匹配（将使用HooksDispatcherOnMount），但要有额外的DEV验证，以确保钩子的顺序没有改变。这个调度器做到了这一点。
      ReactCurrentDispatcher$1.current = HooksDispatcherOnMountWithHookTypesInDEV;
    } else {
      ReactCurrentDispatcher$1.current = HooksDispatcherOnMountInDEV;
    }
  }
  // 执行组件方法 挂载hooks 如果是jsx随后执行jsxWithValidation jsx-dev生成ReactElement
  // 这个时候还未挂载ref,ref此时为useref生成的对象｛current：null｝
...
// 在这个过程中进度渲染阶段更新了吗
  if (didScheduleRenderPhaseUpdateDuringThisPass) {
    ...
    // 只要渲染阶段的更新继续被安排，就一直保持渲染的循环。使用一个计数器来防止无限循环。
    do {
  ...
  // 我们可以假设前一个dispatcher始终是这个dispatcher，因为我们在渲染阶段的开始就设置了它，而且没有重入。
  ...
  // 这个检查使用currentHook，所以它在DEV和prod bundles中的效果是一样的。
  // hookTypesDev可以捕获更多的情况（例如上下文），但只在DEV bundles中。
...

  {
    ...
    // 确认自上次渲染以来没有添加或删除静态标志。如果这时启动，
    // 说明我们在代码库的其他部分错误地重置了静态标志。这种情况以前也发生过，例如在SuspenseList的实现中。
    ...
    // 在legacy mode下禁用这个警告，因为传统的Suspense很奇怪，会产生误报。为了使这一方法在legacy mode下发挥作用，
    // 我们需要以某种方式标记提交不完整状态的fiber。
    // 现在我将禁用这个警告，因为大多数会触发它的错误要么是并发模式所独有的，要么是在两种模式下都存在。
...
function dispatchAction(fiber, queue, action) {
  ...
  if (fiber === currentlyRenderingFiber$1 || alternate !== null && alternate === currentlyRenderingFiber$1) {
    ...
    // 这是一个渲染阶段的更新。把它藏在一个懒散地创建的队列->更新的链接列表的地图中。
    // 在这个渲染过程之后，我们将重新启动并在正在进行的钩子上面应用隐藏的更新。
    // 渲染阶段执行 setstate
    ...
    if (fiber.lanes === NoLanes && (alternate === null || alternate.lanes === NoLanes)) {
      ...
      // 目前队列是空的，这意味着我们可以在进入渲染阶段之前急切地计算下一个状态。
      // 如果新的状态与当前状态相同，我们也许可以完全跳出。
      ...
      if (lastRenderedReducer !== null) {
        ...
        try {
          ...
          // 将急于计算的状态和用于计算的还原器存放在更新对象上。如果在我们进入渲染阶段时，
          // 还原器还没有改变，那么急切的状态就可以被使用，而不需要再次调用还原器。
          ...
          if (objectIs(eagerState, currentState)) {
            ...
            // 快速路径。我们可以在不安排React重新渲染的情况下跳出。
            // 我们仍然有可能在以后需要重新建立这个更新。
            // 如果该组件由于不同的原因重新渲染，而那时的reducer已经改变了。
            return;
          }
...
function pushHostRootContext(workInProgress) {
  ...
  if (root.pendingContext) {
    ...
    // Should always be set 根fiber首次为｛｝
...
function updateHostRoot(current, workInProgress, renderLanes) {
  ...
  cloneUpdateQueue(current, workInProgress); // 将current的更新队列 clone 到workInProgress
  processUpdateQueue(workInProgress, nextProps, null, renderLanes); //更新了updatequeue
  var nextState = workInProgress.memoizedState; //拿到State memoizedState = newState
  var root = workInProgress.stateNode; // 拿到current fiber

  {
    ...
    if (nextCache !== prevState.cache) {
      // The root cache refreshed. 根缓存刷新。
      propagateContextChange(workInProgress, CacheContext, renderLanes);
    }
  ...
  if (root.hydrate && enterHydrationState(workInProgress)) {
    ...
    // root. 开始挂载组件 root节点nextChildren就是render第二个参数element
    // workInProgress.child赋值为reconcileChildFibers函数返回
    reconcileChildren(current, workInProgress, nextChildren, renderLanes);
    resetHydrationState();
  }
...
// 更新HostComponent节点 eg div   typeof string
function updateHostComponent(current, workInProgress, renderLanes) {
  pushHostContext(workInProgress);

  if (current === null) { // 尝试申请下一个Hydratable的实例
  ...
  var nextChildren = nextProps.children;
  var isDirectTextChild = shouldSetTextContent(type, nextProps);  //应设置文本内容 内容是不是文本
  // 避免了分配另一个HostText fiber和遍历它。 文本节点对子节点标记删除，返回null
  if (isDirectTextChild) {
    ...
    workInProgress.flags |= ContentReset; // 之前是不是文本节点 是的话重置
  }
  //标记ref
  markRef(current, workInProgress);
  // 生成child fiber 如果nextChildren是数组，返回首个child fiber，child.sibling指向其他兄弟fiber
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress.child;
}
...
function mountIndeterminateComponent(_current, workInProgress, Component, renderLanes) {
  ...
  if ( // Run these checks in production only if the flag is off. 只有在标志关闭的情况下才能在生产中运行这些检查。
  // Eventually we'll delete this branch altogether.  最终，我们将完全删除这个分支。
   typeof value === 'object' && value !== null && typeof value.render === 'function' && value.$$typeof === undefined) {
    ...
    // During mounting we don't know the child context yet as the instance doesn't exist.
    // We will invalidate the child context in finishClassComponent() right after rendering.
    // 早期推送上下文提供者，以防止上下文栈不匹配。在安装过程中，我们还不知道子上下文，因为实例还不存在。
    ...
    // 生成value的fiber 即函数组件返回值得第一个元素节点 workInProgress.child = child fiber 
...
function validateFunctionComponentInDev(workInProgress, Component) {
  {
    if (Component) {
      if (Component.childContextTypes) { //检查过时api
    ...
    // getDerivedStateFromProps是class组件支持的，调用 render 方法之前调用
    ...
    // 函数组件不存在contextType  class组件存在contextType 
...
function beginWork(current, workInProgress, renderLanes) {
  ...
  if (current !== null) {
    ...
    if (oldProps !== newProps || hasContextChanged() || ( 
      ...
      //  如果由于热重载而导致实现方式改变，则强制重新渲染
     workInProgress.type !== current.type )) {
      // If props or context changed, mark the fiber as having performed work.
      // This may be unset if the props are determined to be equal later (memo).
      // 如果props或context发生了变化，则将fiber标记为已完成工作。
      // 如果后来确定props是相等的（memo），这一点可以不设置。
      didReceiveUpdate = true;
    } else if (!includesSomeLane(renderLanes, updateLanes)) {
      didReceiveUpdate = false; // This fiber does not have any pending work. Bailout without entering
      // the begin phase. There's still some bookkeeping we that needs to be done
      // in this optimized path, mostly pushing stuff onto the stack.
      // 此光纤没有任何未决的工作。无需进入开始阶段就进行退出。
      // 在这个优化的路径中，我们还需要做一些簿记工作，主要是把东西推到堆栈上。
      ...
      if ((current.flags & ForceUpdateForLegacySuspense) !== NoFlags) {
        ...
        // 这个光纤计划进行更新，但没有新的道具或legacy上下文。设置为false。如果更新队列或上下文使用者产生了一个更改的值
        // ，它将将此设置为true。否则，组件将假定子节点没有更改并退出。
        didReceiveUpdate = false;
      }
    }
  } else {
    // 首次工作 除了root fiber会创建current其外都不会存在current
    didReceiveUpdate = false;
  ...
  // 在进入开始阶段之前，清除待更新的优先级。
  // TODO：这假设我们即将评估组件并处理更新队列。然而，
  // 有一个例外。SimpleMemoComponent有时会在开始阶段的后期跳出。
  // 这表明我们应该把这个任务从公共路径中移出，移到每个分支中。。
  ...
  switch (workInProgress.tag) {
    ...
    case HostComponent: // type === 'string'
...

{
  ...
  appendAllChildren = function (parent, workInProgress, needsVisibilityToggle, isHidden) {
    ...
    // children to find all the terminal nodes. 我们只创建了最上面的Fiber，但需要向下递归它的子节点来找到所有终端节点。
    var node = workInProgress.child;

    while (node !== null) {
      if (node.tag === HostComponent || node.tag === HostText) { // Text节点 或者div等
        appendInitialChild(parent, node.stateNode); //直接插入
      } else if (node.tag === HostPortal) ; else if (node.child !== null) {  // 宿主树的根。可以嵌套在另一个节点内。
        node.child.return = node; // 存在child 
        node = node.child;
        continue;
      }
    ...
    case HostComponent:
      {
        ...
        if (current !== null && workInProgress.stateNode != null) {
          ...
          if (_wasHydrated) {
            ...
            // 某些渲染器在初始安装时需要提交时间效果。 例如 DOM 呈现器支持某些元素的自动对焦）确保这类渲染器被安排在以后的工作中。
...

        {
          if (nextState !== null && nextState.dehydrated !== null) {
            if (current === null) {
              ...
              // 如果我们没有current，我们就不应该处于hydration状态。然而，在一些路径中，
              // 我们可能已经重新进入水合状态，然后我们可能在水合状态内。在这种情况下，我们就需要退出它。
              resetHydrationState();

              if ((workInProgress.flags & DidCapture) === NoFlags) {
                // This boundary did not suspend so it's now hydrated and unsuspended.
                // 这个边界没有中止，所以它现在是水合的，没有中止的。
                workInProgress.memoizedState = null;
              ...
              // 如果没有什么暂停，我们需要安排一个效果，将这个边界标记为已经水化，
              // 这样事件就知道它们可以自由地被调用。这也是一个信号，用于重放事件和暂停回调。
              // 如果有事情暂停了，安排一个效果来附加重试监听器。所以我们不妨总是标记这个。
...
function commitBeforeMutationEffects(root, firstChild) {
  focusedInstanceHandle = prepareForCommit(root.containerInfo); // 准备commit focused Elem
  nextEffect = firstChild; // 这一步对当前fiber做commit前的准备，eg：清除根节点内容
...
/** 对自己以及兄弟节点做commit前的准备，最后nextEffect赋值为父级fiber */
function commitBeforeMutationEffects_complete() {
  while (nextEffect !== null) {
    var fiber = nextEffect;

    {
      setCurrentFiber(fiber); // HostRoot清除当前节点内容
        ...
        {
          {
            ...
            clearContainer(root.containerInfo); // 清除
          }
...
function commitBeforeMutationEffectsDeletion(deletion) {
  ...
  // 如果能避免调用doesFiberContain()就更好了，也许我们可以重新利用一个subtreeFlags的位置来代替它？
  // 用它来存储焦点实例在树的哪个部分？这假定我们可以在 "渲染 "阶段安全地确定该实例。
      ...
      case SimpleMemoComponent:
        {
          ...
          // 在这一点上，布局效果已经被销毁了（在突变阶段）。这样做是为了防止同级组件的效果相互干扰，
          // 例如，在同一个提交过程中，一个组件的destroy函数不应该覆盖另一个组件的create函数设置的ref。
          {
...
function commitUnmount(finishedRoot, current, nearestMountedAncestor) {
  onCommitUnmount(current); // onCommitFiberUnmount
...
function detachFiberMutation(fiber) {
  ...
  // 切断返回指针，使其与树断开连接。
  // 这使得我们能够检测并警告未挂载的组件的状态更新。
  // 它还可以防止事件从断开连接的组件中冒出来。

  // 理想情况下，我们也应该清除父代的子代指针，让它被GC:ed，但我们不知道哪个父代是当前的父代，所以我们会选择GC:ing这个子代的子树。
  // 这个子树本身将在父树下次更新时被GC:ed。

  // 注意，我们还不能清除子树或兄弟姐妹的指针。
  // 它们是被动效果和 findDOMNode 所需要的。
  // 我们把这些字段和其他所有的清理工作推迟到被动阶段（见detachFiberAfterEffects）。

  // 也不要重新设置备用的。我们需要这样做，这样我们就可以在被动阶段分离出候补的字段。清除返回指针对于findDOMNode语义来说是足够的。
  var alternate = fiber.alternate;
...
function unmountHostComponents(finishedRoot, current, nearestMountedAncestor) {
  ...
  // children to find all the terminal nodes.
  // 我们只有被删除的顶部Fiber，
  // 但我们需要向下递归它的子节点来找到所有的终端节点。
  var node = current; // Each iteration, currentParent is populated with node's host parent if not
  // currentParentIsValid.
  //  每次迭代，如果不是currentParentIsValid，c
  // urrentParent将被填充为节点的宿主父本。
  ...
  while (true) {
    ...
    if (node.tag === HostComponent || node.tag === HostText) {//commitNestedUnmounts 还会执行想effect卸载
...
function commitDeletion(finishedRoot, current, nearestMountedAncestor) {
  {
    ...
    // 递归地从父节点中删除所有host nodes。
    // 在整个子树上分离 refs 并调用 componentWillUnmount() 。
    unmountHostComponents(finishedRoot, current, nearestMountedAncestor);
  }
  // 切断current与fiber树之间的联系，(gc)
  ...
  while (nextEffect !== null) {
    ...
    if (deletions !== null) {
      for (var i = 0; i < deletions.length; i++) {
        ...
        // 找到需要删除的fiber 删除host nodes 调用componentWillUnmount 以及effect卸载
        {
          // 删除 并且切断与fiber树的联系（gc
    ...
    if ((nextEffect.flags & ChildDeletion) !== NoFlags) {
      ...
      if (deletions !== null) {
...

        {
          ...
          //  一根光纤从这个父光纤中被删除了，但它仍然是先前（alternate）父光纤的子列表的一部分。
          // 因为子代是一个链接列表，仍然活着的早期兄弟姐妹将通过其 "alternate"连接到被删除的光纤。
          //   live fiber
          ...
          // 我们不能断开尚未删除的节点上的`alternate'，但我们可以断开`sibling'和`child'的指针。
...
function ensureCorrectReturnPointer(fiber, expectedReturnFiber) {
  ...
  // 一旦我们确信它不会破坏任何东西，就删除这个赋值，方法是检查上述不变性的警告日志
...
/**
 * 允许组件改变其子树的渲染通道的堆栈 这是我们在根部开始工作的通道的超集。
 * 它与`workInProgressRootRenderLanes`不同的唯一情况是，当我们进入一个隐藏的子树，
 * 需要解除隐藏。悬念和离屏组件。
 * 工作循环中的大多数事情应该处理workInProgressRootRenderLanes.
 * 开始/完成阶段的大多数事情应该处理subtreeRenderLanes。
 */
...
//  只在enableProfilerNestedUpdateScheduledHook为true时使用。
...
function requestEventTime() {
  // executionContext首次render时为4
  // RenderContext 8 CommitContext 16  8 | 16 = 24
  // 24 & 4 =0
  if ((executionContext & (RenderContext | CommitContext)) !== NoContext) {
    // We're inside React, so it's fine to read the actual time.
    // 我们在React里面，所以可以读取实际时间
    return now();
  ...
  // 这是React发布后的第一次更新。计算新的开始时间。

  currentEventTime = now();
  return currentEventTime;
}
function requestUpdateLane(fiber) {
  // Special cases 根节点为0 存在开发工具DevTools 则为2
  var mode = fiber.mode;
  // ConcurrentMode = 1
...
function scheduleUpdateOnFiber(fiber, lane, eventTime) {
  ...
  // 标记从fiber到根的更新通道 返回root.stateNode 也就是根fiber
  // 从父路径走到根路径，并更新子路径。
  // 执行此函数就是从根fiber开始更新
  ...
  if (root === workInProgressRoot) {
    ...
    // 接收到正在渲染中的树的更新。标记此根上有交错更新工作。除非' deferRenderPhaseUpdateToNextBatch '标志关闭，
    // 这是一个渲染阶段更新。在这种情况下，由于向后比较的原因，我们不会将呈现阶段更新视为交错的。
    {
    ...
    if (workInProgressRootExitStatus === RootSuspendedWithDelay) {
      ...
      // 根已经暂停了，这意味着这个渲染肯定不会完成。因为我们有一个新的更新，让我们标记为暂停现在，
      // 就在标记即将到来的更新之前。
      // 这将中断当前渲染并切换到更新。要做的事:确保这不会覆盖发生时，我们已经开始渲染。
  ...
  if (lane === SyncLane) {
    ...
    (executionContext & (RenderContext | CommitContext)) === NoContext) {
      ...
      //  这是一个遗留的边缘案例。ReactDOM.render-ed的初始挂载
      // 的根的初始挂载应该是同步的，但布局更新 应该被推迟到批处理结束。
      ...
      if (executionContext === NoContext && (fiber.mode & ConcurrentMode) === NoMode) {
        ...
        // 现在刷新同步工作，除非我们已经在工作或在批处理中。这是有意在scheduleUpdateOnFiber而不是
        // scheecallbackforfiber中保留调度回调而不立即刷新它的能力。我们只对用户发起的更新这样做，以保留遗留模式的历史行为。
        resetRenderTimer();
...
// on a fiber.
// 这被分割成一个单独的函数，所以我们可以用待定的fiber来标记
// 工作，而不把它当作源于事件的典型更新。
// 例如，重试一个暂停边界不是一个更新，但它会安排工作 但它确实安排了光纤上的工作。
function markUpdateLaneFromFiberToRoot(sourceFiber, lane) {
  // Update the source fiber's lanes  更新源光纤的通道
...
function isInterleavedUpdate(fiber, lane) {
  return (// TODO: Optimize slightly by comparing to root that fiber belongs to.
    ...
    // 通过与纤维所属的根进行比较，稍作优化。需要一些重构。但这不是什么大问题，因为并发应用程序很少有超过一个根的情况。
    workInProgressRoot !== null && (fiber.mode & ConcurrentMode) !== NoMode && ( 
      // If this is a render phase update (i.e. UNSAFE_componentWillReceiveProps),
    // then don't treat this as an interleaved update. This pattern is
    // accompanied by a warning but we haven't fully deprecated it yet. We can
    // remove once the deferRenderPhaseUpdateToNextBatch flag is enabled.
    // 如果这是一个渲染阶段的更新（即UNSAFE_componentWillReceiveProps），那么不要把它当作交错更新。
    // 这种模式伴随着一个警告，但我们还没有完全废止它。一旦启用deferRenderPhaseUpdateToNextBatch标志，我们就可以删除。
...
function ensureRootIsScheduled(root, currentTime) {
  ...
  //检查是否有任何lanes被其他工作starved。如果是这样，把它们标记为过期，这样我们就知道下一步要做这些工作。
  markStarvedLanesAsExpired(root, currentTime); // Determine the next lanes to work on, and their priority.
  // 确定下一步工作的车道，以及它们的优先次序。
  ...
  if (newCallbackPriority === SyncLane) {
    ...
    // internal queue 同步React的回调被安排在一个特殊的内部队列中
    ...
    {
      // Flush the queue in a microtask.  在一个微任务中冲刷队列。
...
function markRootSuspended$1(root, suspendedLanes) {
  ...
  // 当挂起时，我们应该始终排除那些被ping或在渲染阶段更新的车道(更罕见，因为我们试图避免它)。
  // 哈哈，也许除了这个名字很讨厌的函数之外，还有更好的方法来分解这个:)
...
// through Scheduler
//  这是同步任务的入口，这些任务不需要经过通过调度器Scheduler
function performSyncWorkOnRoot(root) {
  ...
  if (root.tag !== LegacyRoot && exitStatus === RootErrored) {
    executionContext |= RetryAfterError; // If an error occurred during hydration,
    // discard server response and fall back to client side render.
    // 如果在hydration过程中发生错误。丢弃服务器响应并返回到客户端渲染。
    if (root.hydrate) {
      root.hydrate = false;
  ...
  // pending level.
  // 在退出之前，确保有一个回调安排给下一个 pending level。
  ensureRootIsScheduled(root, now());
  return null;
}
...
function unbatchedUpdates(fn, a) {
  // 拿到执行上下文 初次为nocontext 0
  var prevExecutionContext = executionContext;
  // ~按位非运算符
  executionContext &= ~BatchedContext // 1;0 & 0 = 0
  // |:或
  executionContext |= LegacyUnbatchedContext //4 100; 4
  ...
  try {
    ...
    // most batchedUpdates-like method.
    //  如果有遗留的同步更新，
    // 则在外部的 "同步更新 "方法结束时刷新它们。最类似于batchedUpdates的方法结束时冲刷它们。
    if (executionContext === NoContext) {
      ...
      // 我认为这个调用是多余的，因为我们在LegacyUnbatchedContext被设置时，
      // 会在里面刷新 scheduleUpdateOnFiber时。
      flushSyncCallbacksOnlyInLegacyMode();
    }
  }
}
...
function prepareFreshStack(root, lanes) {
  ...
  if (timeoutHandle !== noTimeout) {
    ...
    // $FlowFixMe抱怨noTimeout不是一个TimeoutID，尽管上面有检查。
  ...
  // 当前workInProgressRoot
  workInProgressRoot = root;
  // 首次渲染current树不存在alternate
  // 根据current.alternate创建当前createWorkInProgress 
  // current.alternate = workInProgress workInProgress.alternate = current
  // 因为是根 所以无pendingprops
...
function pushDispatcher() {
  ...
  if (prevDispatcher === null) {
    ...
    // React同构包不包含默认分派器。相反，第一个渲染器将惰性地附加一个，以提供更好的错误消息。
...
function renderRootSync(root, lanes) {
  ...
  // prevDispatcher 之前的hooks dispatcher
  var prevDispatcher = pushDispatcher(); 
  // If the root or lanes have changed, throw out the existing stack
  // and prepare a fresh one. Otherwise we'll continue where we left off.
  //如果根或车道发生了变化，就扔掉现有的堆栈，准备一个新的。否则我们就继续上次的内容。
  if (workInProgressRoot !== root || workInProgressRootRenderLanes !== lanes) {
    {
      if (isDevToolsPresent) {
        ...
        // 在这一点上，将安排即将进行的工作的Fibers从Map中移到集合中。
        // 如果我们在这项工作上放弃了，我们将把它们移回来（像上面那样）。
        // 现在移动它们是很重要的，因为万一这项工作在相同的优先级下产生了更多的工作，
        // 而且是不同的更新者。这样，我们就可以把当前的更新和未来的更新分开。
        movePendingFibersToMemoized(root, lanes);
      }
    }
    // 准备新鲜的堆栈 赋值workInProgress后开始工作循环
...
function workLoopSync() {
  ...
  // 已经超时了，所以在没有检查我们是否需要让步的情况下进行工作。
  while (workInProgress !== null) {
    // 执行工作单元 workInProgress根据root.current创建的fiber  or current.alternate
  ...
  if (workInProgressRoot !== root || workInProgressRootRenderLanes !== lanes) {
    {
      if (isDevToolsPresent) {
        ...
        // It's important to move them now in case the work spawns more work at the same priority with different updaters.
        // That way we can keep the current update and future updates separate.
        // 此时，将计划即将进行的工作的纤程从Map移到Set
        // 。如果我们对这项工作进行救助，我们将把他们拉回来(就像上面提到的那样)。
        // 现在移动它们是很重要的，以防工作在不同的更新器中产生更多具有相同优先级的工作。
        // 这样我们就可以将当前的更新和未来的更新分开。
        movePendingFibersToMemoized(root, lanes);
      }
    }
...
function performUnitOfWork(unitOfWork) {
  ...
  // 该纤维当前的刷新状态是备用状态。理想情况下，不应该依赖于此，
  // 但在这里依赖它意味着我们不需要在正在进行的工作中添加额外的领域。
  // unitOfWork 其实就是 workInProgress
  // 拿到current fiber
  var current = unitOfWork.alternate;
  // 设置全局current isRendering=false
  setCurrentFiber(unitOfWork);
  var next;
  // 配置模式isDevToolsPresent为true时
  if ( (unitOfWork.mode & ProfileMode) !== NoMode) {
    //设定开始时间profilerStartTime
    // 设定单元fiber actualStzartTime（实际开始时间）
  ...
  if (next === null) {
    ...
    // 如果这没有产生新的工作，完成当前的工作。 不存在子节点
    // 解析一些功能节点 以及绑定节点独有事件到dispatch，或者是解析ContextProvider类不存在子节点的节点
    // 最后返回兄弟节点，不存在则返回父级兄弟节点 dfs遍历
    completeUnitOfWork(unitOfWork);
  } else {
    // next为 生成的child fiber
    workInProgress = next;
  }
  // 重置current
  ReactCurrentOwner$2.current = null;
  ...
  do {
    ...
    if ((completedWork.flags & Incomplete) === NoFlags) {
      ...
      if (next !== null) {
        ...
        // 完成这个纤维催生了新的工作。接下来要做的就是这个。完成这个纤维催生了新的工作。接下来要做的就是这个。
        workInProgress = next;
        return;
      }
    } else {
    ...
    if (siblingFiber !== null) {
      // If there is more work to do in this returnFiber, do that next.  如果在这个returnFiber中还有更多的工作要做，接下来就做。
...
function commitRoot(root) {
  ...
  // 这已经没有任何意义了。我们已经打包mutation和 布局阶段。应该可以删除。
  var previousUpdateLanePriority = getCurrentUpdatePriority(); //默认0 返回currentUpdatePriority当前更新优先级
  var prevTransition = ReactCurrentBatchConfig$3.transition;

  try {
    ReactCurrentBatchConfig$3.transition = 0;
    // 设置currentUpdatePriority当前更新优先级 为1 即为同步
    setCurrentUpdatePriority(DiscreteEventPriority);
...
function commitRootImpl(root, renderPriorityLevel) {
  do {
    ...
    // `flushPassiveEffects`会在最后调用`flushSyncUpdateQueue`，这意味着`flushPassiveEffects`
    // 有时会导致额外的被动效果。所以我们需要在一个循环中不断刷新，直到没有更多的待处理效果。
    // TODO:  如果`flushPassiveEffects`在最后不自动刷新同步工作，可能会更好，以避免像这样的因果关系危害。
    flushPassiveEffects();
  ...
  // commitRoot从来不会返回一个continuation；它总是同步完成。所以我们现在可以清除这些，以允许安排一个新的回调。

  root.callbackNode = null;
  root.callbackPriority = NoLane; // Update the first and last pending times on this root. The new first
  // pending time is whatever is left on the root fiber.
  // 更新该root上的第一个和最后一个待处理时间。新的第一个 是root fiber上剩下的任何时间。
  // 合并
  ...
  // They're redundant.
  // 如果有待处理的passive effects，安排一个回调来处理它们。
  // 尽可能早地这样做，这样它就会排在其他可能在提交阶段被安排的事情之前。参见#16714）。
  ...
  if ((finishedWork.subtreeFlags & PassiveMask) !== NoFlags || (finishedWork.flags & PassiveMask) !== NoFlags) {
    if (!rootDoesHavePassiveEffects) {
      ...
      scheduleCallback(NormalPriority, function () {
        // 调用useeffect invokeGuardedCallback
        flushPassiveEffects();
        return null;
      });
    }
  ...
  // 检查整个树中是否有任何效果。
  // TODO: 这是从效果列表的实现中遗留下来的，在那里我们必须检查`firstEffect`的存在以满足Flow的要求。
  // 我认为这个优化存在的唯一原因是它会影响到分析。
  // 重新考虑这是否有必要。
  ...
  if (subtreeHasEffects || rootHasEffect) {
    ...
    // 提交阶段被分成几个子阶段。我们对每个阶段的效果列表进行单独处理：
    // 所有mutation effects都在所有布局效果之前，以此类推。第一个阶段是 "变异前 "阶段。
    // 我们用这个阶段来读取宿主树的状态，
    // 然后再对其进行mutate。这就是getSnapshotBeforeUpdate被调用的地方。
    var shouldFireAfterActiveInstanceBlur = commitBeforeMutationEffects(root, finishedWork);

    {
      // Mark the current commit time to be shared by all Profilers in this
      // batch. This enables them to be grouped later.
      // 将当前的提交时间标记为本批中的所有剖析器(Profilers)共享。这使得它们以后可以被分组。
      recordCommitTime();
    }

    // Mutation阶段 遍历所有fiber 
    // 先清除fiber上的deletions 并且触发componentWillUnmount，以及deletions effect卸载 执行useeffect 返回函数
    // 真正的操作dom节点，更新节点 
    ...
    // 正在进行中的树现在是当前的树。这必须在突变阶段之后，这样在componentWillUnmount阶段，之前的树仍然是当前的，
    //  但在布局阶段之前，所以在componentDidMount/Update期间，完成的工作是当前的。。
    root.current = finishedWork; // The next phase is the layout phase, where we call effects that read
    //  下一个阶段是布局阶段，在这个阶段我们调用读取的效果
    {
      markLayoutEffectsStarted(lanes);
    }
    // 执行useLayoutEffect  执行componentDidMount
  ...
  if (rootDoesHavePassiveEffects) {
    ...
    // 这个提交有被动效果。藏起对它们的引用。但在冲刷布局工作之后，不要安排回调。
    rootDoesHavePassiveEffects = false;
    rootWithPendingPassiveEffects = root;
    pendingPassiveEffectsLanes = lanes;
  } // Read this again, since an effect might have updated it
  // 再读一下这个，因为一个效果可能已经更新了
  ...
  // 始终在退出`commitRoot`之前调用这个，以确保任何 额外的工作被安排在这个根上。
  ensureRootIsScheduled(root, now());
  ...
  if ((executionContext & LegacyUnbatchedContext) !== NoContext) {
    ...
    // of the batch.
    // 这是一个遗留的边缘案例。我们刚刚在batchedUpdates中提交了ReactDOM.render-ed root的初始装载。
    // 提交是同步进行的，但布局更新应该被推迟到批处理结束。
    return null;
  ...
  // are consolidated.
  // 如果被动效果是离散渲染的结果，就在当前任务结束时同步冲刷它们，
  // 这样就可以立即观察到结果。否则，我们假设它们不依赖于顺序，
  // 也不需要被外部系统观察到，所以我们可以等到绘制之后再进行。
  // TODO：我们可以通过不提前调度回调来进行优化。因为我们目前在多个地方调度回调，所以将等到这些地方合并。
...
function flushPassiveEffectsImpl() {
  ...
  // 这有时会与rootWithPendingPassiveEffects不同步。弄清楚原因并解决它。
  // 它没有造成任何已知的问题（可能是 因为它只用于分析），但这是一个重构的危险。
  ...
  // 如果安排了额外的被动效果，则递增一个计数器。如果超过了限制，我们会发出警告。
...

{
  ...
  beginWork$1 = function (current, unitOfWork, lanes) {
    ...
    // 如果组件抛出错误，我们将在一个同步分派的事件中再次重新执行它，以便调试器将其视为未捕获的错误。
    // 在进入开始阶段之前，将正在进行的工作复制到一个虚拟光纤上。如果beginWork抛出，我们将使用这个来重置状态。
...
function resolveFunctionForHotReloading(type) {
  {
    if (resolveFamily === null) {
      // Hot reloading is disabled. 关闭热加载功能。
      return type;
    }
...
function createWorkInProgress(current, pendingProps) {
  ...
  if (workInProgress === null) {
    ...
    // 我们使用双缓冲池技术，因为我们知道最多只需要树的两个版本。我们将可以自由重用的“其他”未使用的节点汇聚在一起。
    // 这是惰性创建的，以避免为从不更新的内容分配额外的对象。如果需要，它还允许我们回收额外的内存。
    // 创建fiber
...
function createHostRootFiber(tag, isStrictMode, concurrentUpdatesByDefaultOverride) {
  ...
  if ( isDevToolsPresent) {
    ...
    // 当有DevTools时，总是收集概要文件计时。这使得DevTools可以在任何时间点开始捕捉时间，而不会有树中的某些节点的基本时间是空的。
    mode |= ProfileMode;
  }
...
function updateContainer(element, container, parentComponent, callback) {
  ...
  var current$1 = container.current; //当前根fiber的current fiber
  // 获取react更新开始事件
  var eventTime = requestEventTime();
  ...
  var lane = requestUpdateLane(current$1); // 首次为1 同步

  {
    // 标记 渲染 安排
    markRenderScheduled(lane);
  }
  // 获取子树的上下文 parentComponent 根节点render为null 返回{}
  ...
  // 注意:React DevTools目前依赖于这个称为“element”的属性。
  // being called "element".

  update.payload = {
    element: element //render首个参数 也就是节点或者组件
  };
...
function ReactDOMLegacyRoot(container, options) {
  //LegacyRoot 首次为 0  options =undefined 不是服务端渲染
...
function legacyCreateRootFromDOMContainer(container, forceHydrate) {
  // render首次渲染forceHydrate = false 则获取子节点
  var shouldHydrate = forceHydrate || shouldHydrateDueToLegacyHeuristic(container); // First clear any existing content.
  // 获取节点firstChild 如果是DOCUMENT_NODE类型返回 container.documentElement
  // 首先清除任何现有的内容。
  if (!shouldHydrate) {
    var warned = false;
    var rootSibling;
...

  {
    if (shouldHydrate && !forceHydrate && !warnedAboutHydrateAPI) {
      ...
      // ReactDOM.hydrate() 服务器渲染用
      warn('render(): Calling ReactDOM.render() to hydrate server-rendered markup ' + 'will stop working in React v18. Replace the ReactDOM.render() call ' + 'with ReactDOM.hydrate() if you want React to attach to the server HTML.');
    }
  }
  // 创建根 shouldHydrate服务器渲染用
...
function legacyRenderSubtreeIntoContainer(parentComponent, children, container, forceHydrate, callback) {
  ...
  if (!root) {
    // Initial mount
    // 此处创建根fiber以及current fiber
    // 首次挂载 清理container 在根节点注册_reactRootContainer  
