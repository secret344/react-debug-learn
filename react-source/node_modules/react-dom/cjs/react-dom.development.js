...
function commitRootImpl(root, renderPriorityLevel) {
  do {
    ...
    // 这意味着`flushPassiveEffects`有时会导致额外的工作量。
    // 这意味着`flushPassiveEffects`有时会导致额外的被动效果。
    // 所以我们需要在一个循环中不断刷新，直到没有更多的待处理效果。
    // TODO: 如果`flushPassiveEffects`不在最后自动刷新同步工作，可能会更好，以避免像这样的因子危险。
    // 检查之前未执行的useEffect
    flushPassiveEffects();
  ...
  if (subtreeHasEffects || rootHasEffect) {
    ...
    // 先清除fiber上的deletions 并且触发componentWillUnmount，以及deletions effect卸载
    // 真正的操作dom节点，更新节点 
