...
function advanceTimers(currentTime) {
  ...
  while (timer !== null) {
    if (timer.callback === null) {
      // Timer was cancelled.  计时器被取消了。
      pop(timerQueue);
    } else if (timer.startTime <= currentTime) {
      // Timer fired. Transfer to the task queue. 计时器启动。转移到任务队列中。
      pop(timerQueue); // 删除
...
function shouldYieldToHost() {
  {
    ...
    //  `isInputPending`是不可用的。因为我们没有办法知道是否有待处理的输入，所以总是在帧的末尾产生。
...
var performWorkUntilDeadline = function () {
  if (scheduledHostCallback !== null) {
    ...
    // the message event.
    //  在`yieldInterval`毫秒之后产生，无论我们在帧同步周期中处于什么位置。
    // 这意味着在消息事件的开始总是有剩余的时间。
